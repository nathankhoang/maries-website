...
export async function ensureSchema() {
  ...
  `;
  // Additive migration for tables created before audio support.
  await sql`ALTER TABLE phrases ADD COLUMN IF NOT EXISTS audio_url text`;
}
...
export async function getAllPhrases() {
  const { rows } = await sql`
    SELECT id, category, subsection, en, vi, note, audio_url, sort_order
    FROM phrases
...
export async function getGroupedPhrases() {
  ...
  for (const r of rows) {
    ...
    entry.rows.push({ id: r.id, en: r.en, vi: r.vi, note: r.note, audio_url: r.audio_url });
...
export async function createPhrase({
  ...
  const { rows } = await sql`
    INSERT INTO phrases (category, subsection, en, vi, note, audio_url, sort_order)
    VALUES (${category}, ${subsection}, ${en}, ${vi}, ${note}, ${audio_url}, ${sort_order})
    RETURNING id, category, subsection, en, vi, note, audio_url, sort_order, created_at, updated_at
  `;
  return rows[0];
}
